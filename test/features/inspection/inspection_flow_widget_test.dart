...
void main() {
  ...
  testWidgets('Instanz + Prüfung anlegen über das Gerätedetail', (
    ...
    await tester.pumpWidget(
      buildTestApp(
        db: db,
        home: EquipmentDetailScreen(equipmentId: equipmentId),
      ),
    );
    ...
    expect(
      schedules.single.dueAt.year * 12 + schedules.single.dueAt.month,
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.pumpWidget(
        buildTestApp(db: db, home: const VehicleListScreen()),
      );
      await tester.pumpAndSettle();
      ...
      await tester.pumpAndSettle();
      await tester.enterText(
        find.widgetWithText(TextField, 'Erledigt von (optional)'),
        'Marcus',
      );
