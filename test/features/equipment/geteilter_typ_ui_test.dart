...
void main() {
  ...
  group('Pflege am Gerät ist Pflege am geteilten Typ', () {
    test('anlegen und ändern werden zum Verteilen vorgemerkt', () async {
      ...
      expect(
        (await db.equipmentDao.getById(id))?.typeDirty,
        isTrue,
      ...
      expect(
        ...
        'typ-1',
        reason:
            'die Verbindung zum geteilten Typ darf eine Bearbeitung '
            'überleben',
      );
    });
    ...
    test('der Seeder-Weg am Repository vorbei bleibt still', () async {
      ...
      final id = await db.equipmentDao.insertEquipment(
        EquipmentItemsCompanion.insert(name: 'C-Rohr'),
      );
  ...
  testWidgets('Bearbeiten verliert nicht, was im Formular kein Feld hat', (
    ...
    expect(
      zeile.libraryEquipmentId,
      'std_wbk',
      reason:
          'die Katalog-Herkunft ist die Kennung, über die der Server '
          'Typen zusammenführt',
    );
  ...
  group('die Detailseite sagt, wie weit eine Änderung reicht', () {
    ...
    testWidgets('verbunden: geteilter Bestand der Gesamtwehr', (tester) async {
      ...
      expect(
        find.textContaining('Geteilter Bestand der Gesamtwehr'),
        findsOneWidget,
      );
      await endTestApp(tester);
    });
  ...
  group('Entfernen', () {
    ...
    setUp(() async {
      id = await db.equipmentDao.insertEquipment(
        EquipmentItemsCompanion.insert(name: 'Leitkegel'),
      );
    });
