...
class _CutawayQuizScreenState extends ConsumerState<CutawayQuizScreen> {
    ...
    if (compartmentRows.length < 2) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Das Fahrzeug braucht mindestens 2 Fächer für dieses Quiz.',
            ),
          ),
        );
      }
  ...
  Widget _buildQuestion() {
    ...
    if (_answered) {
      tileStates[q.correctCompartmentId] = const CutawayTileState(
        status: CutawayTileStatus.correct,
      );
      if (_tappedCompartmentId != null &&
          _tappedCompartmentId != q.correctCompartmentId) {
        tileStates[_tappedCompartmentId!] = const CutawayTileState(
          status: CutawayTileStatus.wrong,
        );
      }
    }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const Text(
            'In welchem Fach liegt dieses Gerät?',
