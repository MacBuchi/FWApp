...
void main() {
  ...
  test(
    'Katalog wird geseedet, Demo-Fahrzeug referenziert ihn (idempotent)',
    () async {
  ...
  test(
    'Katalog-Aliasse lösen typische Beladelisten-Schreibweisen auf',
    () async {
      ...
      for (final (raw, expectedLibraryId) in [
        ...
        expect(
          match.kind,
          isIn([MatchKind.exact, MatchKind.alias]),
          reason: '$raw sollte direkt aufgelöst werden',
        );
        final target = equipment.firstWhere(
          (e) => e.id == match.best!.equipmentId,
        );
        expect(
          target.libraryEquipmentId,
          expectedLibraryId,
