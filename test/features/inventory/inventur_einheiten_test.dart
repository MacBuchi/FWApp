...
void main() {
  ...
  setUp(() async {
    ...
    verteiler = await db.equipmentDao.insertEquipment(
      EquipmentItemsCompanion.insert(name: 'Verteiler'),
    );
  ...
  test(
    '⚠️ die Einheit landet in der Zeile IHRES Fachs, nicht in beiden',
    () async {
      ...
      final sessionId = await container
          ...
          .startOrResume(vehicleId);

      final proZeile = await container.read(
  ...
  test('ein entfernter Code taucht im Bericht nicht mehr auf', () async {
    ...
    await db.tagDao.aendere(
      tag!.id,
      EquipmentTagsCompanion(deletedAt: Value(DateTime.now())),
    );
  ...
  test('eine Zeile ohne geführte Einheiten bleibt leer, nicht null', () async {
    final sessionId = await container
        .read(inventoryServiceProvider)
        .startOrResume(vehicleId);
    final proZeile = await container.read(
    ...
    for (final c in checks) {
      expect(
        ...
        isEmpty,
        reason:
            'Jede Zeile bekommt einen Eintrag, damit der Export nicht '
  ...
  test('eine Einheit im Lager gehört zu keiner Zeile', () async {
    await db.inspectionDao.insertInstance(
      EquipmentInstancesCompanion.insert(
        equipmentId: verteiler,
        identifier: const Value('Reserve'),
      ),
    );
    final sessionId = await container
        .read(inventoryServiceProvider)
