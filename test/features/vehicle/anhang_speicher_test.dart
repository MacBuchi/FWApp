...
void main() {
  ...
  Future<int> seedFahrzeug() => db.vehicleDao.insertVehicle(
  ...
  group('im Browser (Issue #210)', () {
    ...
    test('lehnt das Anhängen mit einem Satz ab, nicht mit einer Ausnahme '
      ...
      await expectLater(
        browser.hinzufuegen(
          vehicleId: vehicleId,
          dateiname: 'Anleitung.pdf',
          bytes: pdf,
        ),
        throwsA(
          isA<AnhangAbgelehnt>().having(
            (e) => e.grund,
            'grund',
            AnhangSpeicher.kNurInDerApp,
          ),
        ),
      );
    });
  ...
  group('hinzufügen', () {
    test(
      'legt die Datei lokal ab und trägt sie ein — ohne jeden Server',
      () async {
  ...
  group('sicherstellenLokal', () {
    ...
    test(
      'ein Pfad auf eine gelöschte Datei gilt nicht als vorhanden',
      () async {
  ...
  group('entfernen', () {
    ...
    test(
      'eine bereits verschwundene Datei bricht das Entfernen nicht ab',
      () async {
        final fahrzeug = await seedFahrzeug();
        final anhang = await speicher.hinzufuegen(
          vehicleId: fahrzeug,
