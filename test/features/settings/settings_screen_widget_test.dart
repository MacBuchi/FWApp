...
void main() {
  ...
  testWidgets('Sync deaktiviert: keine Verbindungsfelder sichtbar', (
    ...
    await tester.pumpWidget(buildTestApp(db: db, home: const SettingsScreen()));
  ...
  testWidgets('Sync konfiguriert, aber nicht initialisiert: Neustart-Hinweis', (
    tester,
  ) async {
    SharedPreferences.setMockInitialValues({
      'sync_enabled': true,
  ...
  Future<void> scrollToConnect(WidgetTester tester) =>
  ...
  testWidgets('Server erreichbar: grüner Status vor dem Login', (tester) async {
  ...
  testWidgets('Vorabversionen: Schalter ist da und ab Werk aus', (
    ...
    await tester.scrollUntilVisible(
      find.text('Vorabversionen erhalten'),
      200,
      scrollable: find.byType(Scrollable).first,
    );
  ...
  testWidgets('Vorabversionen: kein Schalter, wo es keine Updates gibt', (
    ...
    try {
      await tester.pumpWidget(
        buildTestApp(db: db, home: const SettingsScreen()),
      );
      await tester.pumpAndSettle();
